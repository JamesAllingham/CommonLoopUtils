...
_use_split_info = version.parse("4.4.0") < version.parse(tfds.version.__version__)
...
class DatasetBuilder(typing_extensions.Protocol):
    ...
    def as_dataset(
        ...
        decoders: Optional[Dict[str, tfds.decode.Decoder]],
    ) -> tf.data.Dataset:
        ...
...
def _shard_read_instruction(
    ...
    *,
    ...
    return tfds.core.ReadInstruction(
        absolute_instruction.splitname, from_=shard_start, to=shard_end, unit="abs"
    )
    ...
    *,
    ...
    if drop_remainder is not None:
        remainder_options = (
            ...
            if drop_remainder
            else RemainderOptions.BALANCE_ON_PROCESSES
        )
        logging.warning(
            "`drop_remainder` is deprecated. Please pass `remainder_options` "
            "instead. `remainder_options` is reset with %s.",
            remainder_options,
        )
    if dataset_info is None:
        if any(special in split for special in ["[", "]", "+"]):
            raise ValueError(f"Sharding split {split} requires passing `dataset_info`.")
    if host_id is None:
...
def create_dataset(
    dataset_or_builder: Union[tf.data.Dataset, DatasetBuilder],
    *,
    split: Union[str, tfds.core.ReadInstruction],
    batch_dims: Sequence[int] = (),
    ...
    Args:
      dataset_or_builder: Dataset builder object with a as_dataset() method. E.g.
        instance of `tfds.core.DatasetBuilder` as returned by `tfds.builder(...)`.
        Or, a `tf.data.Dataset` object.
    ...
    if isinstance(dataset_or_builder, tf.data.Dataset):
        ds = dataset_or_builder
    else:
        ...
        ds = dataset_or_builder.as_dataset(
            split=split,
    ...
    if pad_up_to_batches is not None:
        ...
        ds = pad_dataset(
            ...
            pad_up_to_batches=(
                None if pad_up_to_batches == "auto" else pad_up_to_batches
            ),
...
def create_distributed_dataset(
    dataset_or_builder,
    *,
    split: Union[StrOrReadInstruction, Callable[[int, int], StrOrReadInstruction]],
    ...
    def dataset_fn(input_context: tf.distribute.InputContext):
        ...
        return create_dataset(
            dataset_or_builder=dataset_or_builder,
            split=local_split,
